...
select.samples <- all.samples[J(c("mRNA_3000cpc_Organoid_MD101","mRNA_30cpc_Organoid_MD114","mRNA_30cpc_Str", "mRNA_30cpc_SN","mRNA_30cpc_Th","mRNA_30cpc_Ctx","mRNA_3cpc_Str", "mRNA_3cpc_SN","mRNA_3cpc_Th","mRNA_3cpc_Ctx","mRNA_30cpc_Str_4wks", "mRNA_30cpc_SN_4wks","mRNA_30cpc_Th_4wks","mRNA_30cpc_Ctx_4wks","mRNA_3cpc_Str_4wks","mRNA_3cpc_SN_4wks","mRNA_3cpc_Th_4wks","mRNA_3cpc_Ctx_4wks"))] 
...
select.samples[Animals=="mRNA_3000cpc_Organoid_MD101", BCcount := as.integer(BCcount/5),] # Removes unclear 3000cpc reads
...
write.fasta(as.list(select.samples.trsp$Peptide), fasta.names, "data/invivoSamplesPeptidesOrganoids.fasta", open = "w", nbchar = 60, as.string = TRUE)
...
sys.out <-  system(paste("java -jar /home/rstudio/Hammock_v_1.1.1/dist/Hammock.jar full -i /home/rstudio/data/invivoSamplesPeptidesOrganoids.fasta -d /home/rstudio/data/HammockInVivoOrganoids --max_shift 7 -c 250 --alignment_threshold 26 --assign_thresholds 50,40,30 -t ", detectCores(), sep = ""),
...
#'===================
ham.clusters <- data.table(read.table("/home/rstudio/data/HammockInVivoOrganoids/final_clusters.tsv", header = TRUE, skip = 0, sep="\t",
                                      stringsAsFactors = FALSE, fill=TRUE))
id.order <- as.list(ham.clusters$cluster_id)
ham.clusters.all <- data.table(read.table("/home/rstudio/data/HammockInVivoOrganoids/final_clusters_sequences.tsv", header = TRUE, skip = 0, sep="\t",
                                          stringsAsFactors = FALSE, fill=TRUE))
...
dir.create(file.path("/home/rstudio/data/HammockInVivoOrganoids/", "alignments_final_Scored"), showWarnings = FALSE)
...
generateWeblogo <- function(in.name) {
  ...
  # in.name <- 6777
  this.fa <- read.fasta(file = paste("/home/rstudio/data/HammockInVivoOrganoids/alignments_final/", in.name, ".aln", sep=""))
  ...
  write.fasta(as.list(allSeqs.out$Alignment), allSeqs.out$Annot, nbchar = 60, paste("/home/rstudio/data/HammockInVivoOrganoids/alignments_final_Scored/", in.name, ".aln", sep=""), open = "w")
  ...
  tmp <- system(paste("weblogo --format PDF --sequence-type protein --size large --errorbars NO --resolution 299 --composition equiprobable --color-scheme chemistry --title '",this.title,"' < /home/rstudio/data/HammockInVivoOrganoids/alignments_final_Scored/", in.name, ".aln > /home/rstudio/data/WEBlogosInVivo/",in.name,".pdf", sep = ""),
...
select.samples <- all.samples[J(c("mRNA_3000cpc_Organoid_MD101","mRNA_30cpc_Organoid_MD114","mRNA_3cpc_HEK293T", "mRNA_30cpc_HEK293T","mRNA_3cpc_pNeuron","mRNA_30cpc_pNeuron"))] # "mRNA_3000cpc_Organoid_MD101"
...
fasta.names <- paste(1:nrow(select.samples.trsp),select.samples.trsp$Score,select.samples.trsp$Group, sep = "|")
write.fasta(as.list(select.samples.trsp$Peptide), fasta.names, "data/invitroSamplesPeptidesOrganoids.fasta", open = "w", nbchar = 60, as.string = TRUE)
...
sys.out <-  system(paste("java -jar /home/rstudio/Hammock_v_1.1.1/dist/Hammock.jar full -i /home/rstudio/data/invitroSamplesPeptidesOrganoids.fasta -d /home/rstudio/data/HammockInVitroOrganoids --max_shift 7 -c 50 -t ", detectCores(), sep = ""),
                   intern = TRUE, ignore.stdout = TRUE) 
# Alternative parameters  --use_clinkage --alignment_threshold 23 --max_shift 13 --max_aln_length 37 --count_threshold 50 --max_inner_gaps 0 --assign_thresholds 14.1,10.5,7.0 
...
#'Generation of Weblogo visualization
#'===================
ham.clusters <- data.table(read.table("/home/rstudio/data/HammockInVitroOrganoids/final_clusters.tsv", header = TRUE, skip = 0, sep="\t",
                                      stringsAsFactors = FALSE, fill=TRUE))
id.order <- as.list(ham.clusters$cluster_id)
ham.clusters.all <- data.table(read.table("/home/rstudio/data/HammockInVitroOrganoids/final_clusters_sequences.tsv", header = TRUE, skip = 0, sep="\t",
                                          stringsAsFactors = FALSE, fill=TRUE))
ham.clusters.all[,alignment := gsub('\\-', '\\_', alignment)]
setkey(select.samples,Peptide)
setkey(select.samples.trsp,Peptide)
...
dir.create(file.path("/home/rstudio/data/HammockInVitroOrganoids/", "alignments_final_Scored"), showWarnings = FALSE)
...
generateWeblogo <- function(in.name) {
  #in.name <- ham.clusters$cluster_id[12]
  # in.name <- 6777
  this.fa <- read.fasta(file = paste("/home/rstudio/data/HammockInVitroOrganoids/alignments_final/", in.name, ".aln", sep=""))
  allSeqs <- unlist(getSequence(this.fa, as.string = TRUE))
  allSeqs <- data.table(unlist(lapply(allSeqs, function(x) gsub("([-])","",toupper(x)))))
  allSeqs.out <- select.samples.pepMerge[J(allSeqs)]
  allSeqs.out$Annot <- data.table(getName(this.fa))
  ...
  write.fasta(as.list(allSeqs.out$Alignment), allSeqs.out$Annot, nbchar = 60, paste("/home/rstudio/data/HammockInVitroOrganoids/alignments_final_Scored/", in.name, ".aln", sep=""), open = "w")
  ...
  tmp <- system(paste("weblogo --format PDF --sequence-type protein --size large --errorbars NO --resolution 299 --composition equiprobable --color-scheme chemistry --title '",this.title,"' < /home/rstudio/data/HammockInVitroOrganoids/alignments_final_Scored/", in.name, ".aln > /home/rstudio/data/WEBlogosInVitro/",in.name,".pdf", sep = ""),
                intern = TRUE, ignore.stdout = FALSE)
}
