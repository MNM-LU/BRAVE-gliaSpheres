...
select.samples <- all.samples[J(c("mRNA_3000cpc_Organoid_MD101","mRNA_30cpc_Organoid_MD114","mRNA_30cpc_Str","mRNA_3cpc_Str","mRNA_30cpc_SN","mRNA_30cpc_Th","mRNA_30cpc_Ctx","mRNA_3cpc_SN","mRNA_3cpc_Th","mRNA_3cpc_Ctx","mRNA_30cpc_SN_4wks","mRNA_30cpc_Th_4wks","mRNA_30cpc_Ctx_4wks","mRNA_3cpc_SN_4wks","mRNA_3cpc_Th_4wks","mRNA_3cpc_Ctx_4wks"))] 
...
write.table(out.table, file='data/selectedPeptides_wOrganoids.tsv', quote=FALSE, sep='\t', row.names = FALSE)
...
write.fasta(as.list(select.samples.trsp$Peptide), fasta.names, "data/trspSamplesPeptidesNonSelectOrganoids.fasta", open = "w", nbchar = 60, as.string = TRUE)
...
unlink("/home/rstudio/data/HammockSelectedOrganoids", recursive = TRUE, force = FALSE)
sys.out <-  system(paste("java -jar /home/rstudio/Hammock_v_1.2.0/dist/Hammock.jar cluster -i /home/rstudio/data/selectedPeptides_wOrganoids.tsv -as /home/rstudio/data/trspSamplesPeptidesNonSelectOrganoids.fasta -d /home/rstudio/data/HammockSelectedOrganoids --max_shift 9 --assign_thresholds 15,12,10 --max_aln_length 42 -c ",nrow(selected.peptides)," -t ", detectCores(), sep = ""),
                   intern = TRUE, ignore.stdout = TRUE) 
# Alternative parameters  --use_clinkage --alignment_threshold 23 --max_shift 13 --max_aln_length 42 --count_threshold 50 --max_inner_gaps 0 --assign_thresholds 14.1,10.5,7.0   --assign_thresholds 15,12,10 
hammock.log <- data.table(readLines("data/HammockSelectedOrganoids/run.log"))
...
#'===================
ham.clusters <- data.table(read.table("/home/rstudio/data/HammockSelectedOrganoids/final_clusters.tsv", header = TRUE, skip = 0, sep="\t",
                                      stringsAsFactors = FALSE, fill=TRUE))
id.order <- as.list(ham.clusters$cluster_id)
ham.clusters.all <- data.table(read.table("/home/rstudio/data/HammockSelectedOrganoids/final_clusters_sequences.tsv", header = TRUE, skip = 0, sep="\t",
                                          stringsAsFactors = FALSE, fill=TRUE))
...
dir.create(file.path("/home/rstudio/data/HammockSelectedOrganoids/", "alignments_final_Scored"), showWarnings = FALSE)
...
generateWeblogo <- function(in.name) {
  ...
  this.fa <- read.fasta(file = paste("/home/rstudio/data/HammockSelectedOrganoids/alignments_final/", in.name, ".aln", sep=""))
  ...
  write.fasta(as.list(allSeqs.out$Alignment), allSeqs.out$Annot, nbchar = 60, paste("/home/rstudio/data/HammockSelectedOrganoids/alignments_final_Scored/", in.name, ".aln", sep=""), open = "w")
  ...
  tmp <- system(paste("weblogo --format PDF --sequence-type protein --size large --stacks-per-line 48 --errorbars NO --resolution 299 --composition equiprobable --color-scheme chemistry --title '",this.title,"' < /home/rstudio/data/HammockSelectedOrganoids/alignments_final_Scored/", in.name, ".aln > /home/rstudio/data/WEBlogosSelected/",in.name,".pdf", sep = ""),
